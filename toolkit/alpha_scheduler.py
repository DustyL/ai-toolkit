...
class PhaseAlphaScheduler:
    ...
    def _validate_alpha_ratios(self):
        ...
        if linear_scale > 0.5:
            logger.warning(
                f"Linear alpha scale is HIGH: {self.linear_alpha}/{self.rank} = {linear_scale:.3f}\n"
        ...
        for phase in self.phases:
            ...
            if conv_scale > 0.5:
                logger.warning(
                    f"Conv alpha scale in '{phase.name}' phase is HIGH: {phase.alpha}/{self.rank} = {conv_scale:.3f}\n"
                    f"   This exceeds 0.5 (half of rank). Common practice is scale <= 1.0.\n"
        ...
        if self.per_expert_phases:
            for expert_name, expert_phases in self.per_expert_phases.items():
                for phase in expert_phases:
                    ...
                    if conv_scale > 0.5:
                        logger.warning(
                            f"Conv alpha scale for '{expert_name}' in '{phase.name}' phase is HIGH:\n"
    ...
    def _transition_to_next_phase(self):
        ...
        logger.info(
            ...
            f"  {old_phase.name} (alpha={old_phase.alpha}) -> {new_phase.name} (alpha={new_phase.alpha})\n"
    ...
    def log_status(self):
        ...
        logger.info(
            ...
            f"  Conv: alpha={status['current_conv_alpha']} (scale={status['current_conv_scale']:.6f})\n"
            f"  Linear: alpha={status['current_linear_alpha']} (scale={status['current_linear_scale']:.6f})\n"
